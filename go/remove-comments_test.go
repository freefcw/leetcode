...
func Test_remove_comments(t *testing.T) {
	...
	for _, item := range tables {
		actual := removeComments(item.input)
		if len(actual) != len(item.expect) {
			// t.Errorf("%v expect %d, actual %d\n", item.input, len(item.expect), len(actual))
			t.Errorf("expect %#v, actual %#v\n", item.expect, actual)
			continue
		}
		for i := 0; i < len(actual); i++ {
			if actual[i] != item.expect[i] {
				t.Errorf("%v expect %s, actual %s", item.input, item.expect[i], actual[i])
			}
		}
	}
}
...
func Test_Complex(t *testing.T) {
	...
	actual := removeComments(item.input)
	if len(actual) != len(item.expect) {
		t.Errorf("length not match!%#v\n %#v\n", actual, item.expect)
	}
	for i := 0; i < len(actual); i++ {
		if actual[i] != item.expect[i] {
			t.Errorf("%s expect %s, actual %s", item.input, item.expect[i], actual[i])
		}
	}
}
